...
const referenceMap = { slc: ['SLC-96 reference diagrams','/study/assets/reference-diagrams/slc_96_mode2.pdf'], air: ['Air-pressure reference diagram','/study/assets/reference-diagrams/air-pressure.pdf'], loading: ['Load-coil reference notes','/study/assets/reference-diagrams/loading.pdf'], current: ['Blank-paper drill reference',null] };
const dialog = document.querySelector('#referenceDialog'); document.querySelectorAll('[data-reference]').forEach(button => button.addEventListener('click', () => { const key = button.dataset.reference, item = referenceMap[key]; const source = key === 'current' ? `/study/assets/reference-diagrams/${document.querySelector('#drawPrompt').value}` : item[1]; document.querySelector('#referenceTitle').textContent = item[0]; document.querySelector('#referenceFrame').src = source; dialog.showModal(); })); document.querySelector('#closeReference').addEventListener('click', () => dialog.close());
...
function renderSourceLibrary() {
  ...
  document.querySelector('#sourceLibrary').innerHTML = visible.map(file => `<a class="source-document" target="_blank" href="/study/assets/source-pdfs/${encodeURIComponent(file)}"><b>${prettyFilename(file)}</b><span>${sourceGroup(file)} reference PDF</span></a>`).join('') || '<p>No source documents match that search.</p>';
...
document.querySelector('#openRulesTranscript').addEventListener('click', () => openLibraryReference('All 31 company rules', '/study/assets/rules-transcript.txt'));
document.querySelector('#openKaraoke').addEventListener('click', () => openLibraryReference('Rules karaoke study video', '/study/assets/rules-karaoke.mp4'));
